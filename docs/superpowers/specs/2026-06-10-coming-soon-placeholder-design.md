...
- Impulse model (final, Brendan's call): elasticity comes from cursor MOTION, not a big static field. A stationary cursor leaves only a ~4 CSS px dimple (push 1–2 dither cells within 0.035 × panel height, pow-2 smoothstep). Cursor movement injects velocity impulses into grains within 0.08 × panel height — kick = 0.45 × cursor speed (capped 60 device px/frame) × hashed 0.4–1.2 — so splash distance scales with stroke speed. Springs are stiff and fast (k 0.14–0.24, damp 0.78–0.85): large spatial overshoot, quick settle. "Exaggerated from the distance, not how long the animation is." Superseded tunings: static holes at R 0.32/0.20/0.10 × panel height.
...
- Reverse parallax: REMOVED (Brendan, 2026-06-10 — "stop the slight parallax on the interaction"). The sky is positionally static; only the grains move.
